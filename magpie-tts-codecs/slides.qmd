...
- Innovations and tricks:
  ...
  - **Classifier-free guidance (CFG)** — drop text and prompt in training, modify logits at inference
  - **Group Relative Policy Optimization (GRPO)** — RL post-training, group mean replaces the critic
...
## Multi-Receptive Field Fusion (Hifi-GAN)

![](images/hifigan-mrf.png){fig-align="center" width="70%"}
...
![](images/hifigan-mrf.png){.nostretch fig-align="center" width="45%"}
...
- **Multi-Scale Discriminator** — different scale, i.e. receptive field: raw audio, ×2 avg-pooled, ×4 avg-pooled
- **Multi-Period Discriminator** — different periods, usually primes $[2, 3, 5, 7, 11]$, so it can analyse different periodic patterns in the audio
...
- **Multi-Scale STFT Discriminator** — processes audio with STFTs of different window lengths $[2048, 1024, 512, 256, 128]$, hop size derived from the window size
...
- Other variations:
  - **Multi-Resolution Discriminator (MRD)** — same STFT resolutions, but magnitude only, phase ignored
  - **Multi-Scale Mel Discriminator** — better for human perception, ignores phase
  - **Multiple bands** — adds another dimension, frequency bands, so each discriminator only looks at specific frequencies
...
- Evaluates downstream TTS quality with both AR and NAR FastPitch variants
...
- **86.1 tokens/s** across 8 parallel codebooks (size 1 k each) → 6.9 kbps
...
![](images/spectral-codec-arch.png){fig-align="center" width="80%"}

## Spectral Codec — Details

<div style="display: flex; gap: 1.5em; margin-bottom: 1em;">
<div style="flex: 6; background: #f0f4f8; border-radius: 12px; padding: 0.8em 1.2em;">
...
- **FSQ**: 8 parallel codebooks, each quantizing 4 dims into $[8, 5, 5, 5]$ levels
- **RVQ**: 8 codebooks of size 1024

</div>
<div style="flex: 4; background: #f0f4f8; border-radius: 12px; padding: 0.8em 1.2em;">
...
<div style="background: #f0f4f8; border-radius: 12px; padding: 0.8em 1.2em;">
...
![](images/spectral-codec-results.png){fig-align="center"}

- **Upper bound**: HiFi-GAN without quantization (oracle reconstruction)
- Spectral codecs excel on spectrogram-based metrics; waveform codecs on waveform-based
- RVQ vs FSQ downstream comparison is unfair: RVQ uses parallel prediction heads
- Discrete pipeline (FastPitch → codec decoder) **outperforms** the classic continuous baseline
...
- **Flattening** — all codebooks serialised → $4n$ steps, slowest but fully autoregressive
...
- **Coarse First** — a **single model**: codebook 1 is generated for the whole sequence first, then the remaining codebooks in parallel → $2n$ steps
  - **Coarse First** $\neq$ **VALL-E**, which realises the same coarse-to-fine idea with **two models** — AR for codebook 1 + NAR for the rest, in $n + (4 - 1)$ steps
...
- **Partial Flattening** — a reorganised Coarse First pattern: after the first codebook, all remaining codebooks are predicted in the **next single step**
- **Partial Delay** — Delay + Parallel patterns: after the first codebook, the rest are predicted **in parallel** (Moshi)

<div style="background: #f0f4f8; border-radius: 12px; padding: 0.8em 1.2em;">

Except for the Parallel pattern, the model input at time $t$ is **not** the audio representation at $t$ — it is simply the **sum of the previous predictions** at $t-1$, laid out according to the pattern. For the Delay pattern:
...
- Given that the audio token sequence is longer than the text token sequence, the CTC loss can be computed on **all cross-attention matrices** → penalising the model for **non-monotonic** attention
...
![](images/t5-ablations.png){.nostretch fig-align="center" width="45%"}

- Authors experimented with **where to place the audio prompt**: encoder or decoder
- There is no version with $\mathcal{L}_{align}$ (CTC loss) but **without the prior** — results were **unintelligible**, even though the attention was monotonic

![](images/t5-codec-comparison.png){.nostretch fig-align="center" width="45%"}
...
- For the baselines the authors took **pretrained models with official checkpoints**
- **WER evaluation** — 100 **challenging texts** and 2 **seen speakers** (male and female)
- **MOS evaluation** — 100 sentences from the **Harvard Sentences**, each audio rated by at least **10 listeners** → **2 k** evaluations per model
...
- **Reused from the Spectral Codec:**
  ...
  - Retains the same adversarial **discriminators**
- **Key modifications:**
...
**LSFC Encoder:** HiFi-GAN **MRF** blocks with **dilation rate 1**, each followed by a strided **1D conv** for downsampling → strides mirror the decoder's upsampling
...
- Encoder–decoder **T5-TTS** trained on **1.8 k hours** of English
...
- The strongest **DAC** model is only good at reconstruction — it was never optimized for TTS
...
- Builds upon the **LFSC** with a few modifications
  ...
  - Adds a **speaker consistency loss** — maximise the cosine similarity between speaker embeddings of the real and reconstructed audio:
...
- Finally, **12.5 fps with more FSQ layers** vs **21.5 fps with fewer layers** turn out comparable on all metrics

## Nano Codec — Zero-Shot TTS Results

![](images/nano-codec-tts-results.png){.nostretch fig-align="center" width="80%"}

- **Koel-TTS** — encoder–decoder TTS similar to T5, trained with **CFG** but without **DPO** (RL); a separate model is trained per codec on **18 k hours** of English
- Results are **better or comparable to the LFSC** with a much smaller latency (**TTFA**)
