...
- **Multi-Period Discriminator** — different periods, usually primes $[2, 3, 5, 7, 11]$, so it can analyze different periodic patterns in the audio
...
Except for the Parallel pattern, the model input at time $t$ is **not** the audio representation at $t$ — it is simply the **sum of the previous predictions** at $t-1$, according to the pattern. For the Delay pattern:
