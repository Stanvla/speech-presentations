...
- 🇬🇧 **18k** h + multilingual CML data: 🇩🇪 1.5k h, 🇳🇱 642 h, 🇪🇸 476 + 42 h, 🇫🇷 283 h, 🇮🇹 131 h
...
- **Inference** is autoregressive, token by token — at **each step** we run **two forward passes**:
  1. **Conditioned** — full input (A+T conditioning + previously generated tokens) → $\ell_c$
...
- CFG on a **preference-aligned** model gives the best results — the two techniques are **additive**
...
- This objective has a **closed-form optimal policy** derived from the **constrained** problem — maximize subject to $\sum_y \pi(y \mid x) = 1$ — with a **Lagrange multiplier**, then setting the gradient with respect to $\pi$ to zero:
...
- **Finally**, apply standard **Bradley–Terry** preference model, where only the **reward difference** matters:
...
- With $\pi_y = \pi(y \mid x)$, $q_y = \pi_{\text{ref}}(y \mid x)$, $r_y = r(x, y)$, and a **Lagrange multiplier** enforcing $\sum_y \pi_y = 1$:

  $$\mathcal{L} = \sum_y \pi_y r_y - \beta \sum_y \pi_y \log\frac{\pi_y}{q_y} + \lambda\Big(\sum_y \pi_y - 1\Big)$$

- **Differentiate** w.r.t. each $\pi_y$ and set to zero, using $\frac{\partial}{\partial \pi_y}\!\big[\pi_y \log\tfrac{\pi_y}{q_y}\big] = \log\tfrac{\pi_y}{q_y} + 1$:
...
- **Enforce** $\sum_y \pi_y = 1$ to pin down $C$:

  $$1 = C \sum_y q_y\, e^{r_y/\beta} \;\Longrightarrow\; C = \frac{1}{\sum_y q_y\, e^{r_y/\beta}}$$

- **Define** the partition function $Z(x)$, so $C = 1/Z(x)$:

  $$Z(x) = \sum_y \pi_{\text{ref}}(y \mid x)\, e^{r(x, y)/\beta}$$
